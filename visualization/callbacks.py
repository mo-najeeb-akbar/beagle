...
VizCallback = Callable[[Any, Any, int], None]
...
def create_viz_callback(
    viz_fn: Callable[[Any, Any], dict[str, jnp.ndarray]],
    config: VizConfig,
    ...
    Args:
        viz_fn: Pure function (state, batch) -> dict[str, arrays]
    ...
    Returns:
        Callback with signature: (state, batch, epoch) -> None
    ...
    Example:
        ...
        >>> 
        >>> # In training loop:
        >>> callback(state, batch, epoch)
    """
    def callback(state: Any, batch: Any, epoch: int) -> None:
